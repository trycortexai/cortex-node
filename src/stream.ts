...
>(
      ...
      await readSSE(response, (event, data) => {
            if (!event) {
                  throw new CortexStreamError(
                        `Event is missing in stream for ${type}`,
                  );
            }
            ...
            switch (event) {
                  ...
                  case 'chunk': {
                        ...
                        const output = (data.output ??
                              {}) as StepModelOutputSchema;

                        // loops support
                        if (typeof output.index !== 'undefined') {
                              const step =
                                    currentOutput as CastRunStepOutputSchema<
                                          StepModelOutputSchema[]
                                    >;

                              const stepOutput = step.output?.find(
