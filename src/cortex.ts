...
export class Cortex {
      ...
      private api = async (
            ...
            const methodUpperCase =
                  method.toUpperCase() as keyof APIFetchClient;
            ...
            const streamParser = isStream
                  ? (STREAM_PARSERS[
                          endpoint as keyof typeof STREAM_PARSERS
                    ] as (
                          stream: unknown,
                          event: string,
                          data: unknown,
            ...
            if (isStream && isPagination) {
                  throw new CortexError(
                        'Pagination is not supported with stream',
                  );
            }

            if (isStream && !streamParser) {
                  throw new CortexError(
                        `Stream is yet available for ${endpoint}`,
                  );
            }
            ...
            if (!method) {
                  throw new CortexError(
                        `Method ${method} not found for ${endpoint}`,
                  );
            }
            ...
            if (isStream) {
                  ...
                  const httpStream =
                        name === 'streamResponse' ? new HttpStream() : null;

                  const sse = readSSE(response, (event, data) => {
                        if (!event) {
                              throw new CortexError(
                                    `Event is missing in stream for ${endpoint}`,
                              );
                        }
